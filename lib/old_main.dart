...
String _playerText = 'Player 1 plays now!';
...
class _BoardState extends State<Board> {
  @override
  String _shownText = _playerText;
  void changeText(){
    setState(() => _shownText = _playerText);
  }
  Widget build(BuildContext context) {
    changeText();
    return Column(
...
class _WhiteBoxState extends State<WhiteBox> {
  ...
  void checkWin(){
    ...
    for(List<List<int>> _comb in _winning_comb){
      ...
      if(sum == 3){
        _isNotOccupied = false;
        _playerText = 'Player1 won!';
      } else if(sum == -3){
        _isNotOccupied = false;
        _playerText = 'Player2 won!';
      }
    }}
  @override
  Widget build(BuildContext context) {
